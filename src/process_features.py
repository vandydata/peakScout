...
def get_nearest_features(roi: pl.DataFrame, 
                         feature: str, 
                         starts: pl.DataFrame, 
                         ends: pl.DataFrame, 
                         up_bound: int, 
    ...
    for peak in return_roi.iter_rows(named=True):
        ...
        c_starts_sub, c_ends_sub, c_start_features, c_end_features = constrain_features(peak_start, peak_end,
                                                                                        starts_sub, ends_sub,
        ...
        overlap_features, overlap_index = find_overlaps(peak_start, peak_end, c_starts_sub, 
                                                     overlap_features, overlap_index)
        ...
        while i > 0 and us_index > -1 and ds_index < len(c_start_features):
            ...
            if ds_dist < us_dist:
                update_to_add(features_to_add, dists_to_add, c_start_features, ds_dist, k-i+1, ds_index)
                ds_index += 1
            else:
                update_to_add(features_to_add, dists_to_add, c_end_features, -1 * us_dist, k-i+1, us_index)
                us_index -= 1
    ...
    return gen_return_roi(return_roi, feature, features_to_add, dists_to_add, k)

def constrain_features(peak_start: int, 
                       peak_end: int, 
                       starts: np.ndarray, 
                       ends: np.ndarray, 
                       ...
                       up_bound: int, 
                       down_bound: int) -> tuple[np.ndarray, np.ndarray,
...
def check_overlap(peak_start: int, 
                  peak_end: int, 
                  starts: np.ndarray, 
                  index: int) -> bool:
    '''
...
def find_overlaps(peak_start: int, 
                  peak_end: int, 
                  starts: np.ndarray, 
                  overlap_features: list, 
                  overlap_index: int) -> tuple[list, int]:
    '''
    ...
    overlap_index (int): Index of the first feeature in starts that begins after the current peak ends.
...
def gen_return_roi(return_roi: pl.DataFrame,
                   feature: str,
    ...
    features_to_add (dict): Dicitionary that maps integer n with a list of the nth closest feature.
...
def gen_init(k: int) -> tuple[dict, dict]:
    '''
    Generates dictionaries to add to Polars DataFrame skeletons for nearest feature information.
...
def update_to_add(add_features: dict,
                  add_dists: dict,
                  features: np.ndarray, 
                  dist: int, 
                  add_index: int, 
    ...
    Parameters:
    add_features (dict): Dicitionary that maps integer n with a list of the nth closest feature.
